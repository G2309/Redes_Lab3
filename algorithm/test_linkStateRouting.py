...
class Node:
    ...
    def __init__(self, id, neighbors, network, ttl =10):
        ...
        if isinstance(neighbors, list):
            ...
            for neighbor_id, neighbor_info in neighbors.items():
                if isinstance(neighbor_info, dict) and "host" in neighbor_info:
                    self.neighbors[neighbor_id] = {
    ...
    def send_own_lsa_package(self):
        ...
        for neighbor_id, neighbor_info in self.neighbors.items():
            try:
                if hasattr(self.network, 'publish'):
                    # Para NetworkNodePubSub
                    import asyncio
                    asyncio.create_task(self.network.publish(neighbor_id, lsa_packet))
                else:
    ...
    async def handle_message(self, raw_message, from_neighbor=None):
        with self.lock:
            try:
            ...
            elif message.get("type") == "DATA" or message.get("type") == "message":
    ...
    async def handle_lsa(self, lsa):
        ...
        # Reenviar si TTL > 1
        if ttl > 1:
            await self.forward_lsa(lsa)
        
        self.build_routing_table()

    async def forward_lsa(self, original_lsa):
        ...
        for neighbor_id, neighbor_info in self.neighbors.items():
            ...
            if neighbor_id != original_lsa["source"]:
                try:
                    if hasattr(self.network, 'publish'):
                        await self.network.publish(neighbor_id, forwarded_lsa)
                    else:
                        self.network.send_message(
                            neighbor_info["host"], 
                            neighbor_info["port"], 
    ...
    async def handle_data_message(self, message):
        source = message.get("from", message.get("source"))
        destination = message.get("to", message.get("destination"))
        ...
        if destination in self.routing_table:
            ...
            if next_hop in self.neighbors:
                ...
                try:
                    if hasattr(self.network, 'publish'):
                        await self.network.publish(next_hop, message)
                    else:
    ...
    async def send_data_message(self, destination, payload):
        if destination in self.routing_table:
            ...
            if next_hop in self.neighbors:
                ...
                try:
                    if hasattr(self.network, 'publish'):
                        await self.network.publish(next_hop, data_packet)
                    else:
                        neighbor_info = self.neighbors[next_hop]
                        self.network.send_message(
                            neighbor_info["host"], 
